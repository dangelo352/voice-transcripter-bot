...
@bot.hybrid_command(name="usa", description="Scan Fameswap screenshots (attach images or upload)")
@app_commands.describe(image="Fameswap screenshot image")
async def usa(ctx, image: discord.Attachment = None):
    """Scan Fameswap images and look up all TikTok accounts.
    Works with !usa (attach multiple files) or /usa (upload image)."""
    # Collect images from slash param + prefix command attachments
    images = []
    if image and (image.content_type or "").lower().startswith("image/"):
        images.append(image)
    if ctx.message and hasattr(ctx.message, 'attachments'):
